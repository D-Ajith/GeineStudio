...
export default function DomeGallery({
  /**
   * Leave undefined (the normal case) and the gallery loads its images from
   * /api/gallery, managed at /admin/gallery. Passing an array still works and
   * skips the fetch entirely, so existing call sites keep their behaviour.
   */
  images,
  fit = 0.5,
  fitBasis = 'auto',
  ...
  // Images managed from /admin/gallery. Skipped entirely when a caller passes
  // an `images` prop.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setFetchedImages(rows.map((row) => ({ src: row.image_url, alt: row.alt || '' })));
      } catch {
  ...
  const activeImages = images || fetchedImages;

  const items = useMemo(() => buildItems(activeImages, segments), [activeImages, segments]);
